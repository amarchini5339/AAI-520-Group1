...
class YahooFinanceAgent:
    ...
    def _fetch_price_history(self, symbol, period_days=365, retries=3, delay=2):
        ...
        for attempt in range(retries):
            try:
                # First try yf.download (preferred for full history)
                df = yf.download(
                    symbol,
            ...
            # Retry with .history() as fallback
            try:
                t = yf.Ticker(symbol)
        ...
        raise ValueError(f"Failed to fetch Yahoo Finance price history for {symbol}: {last_err}")
...
def __main():
    ...
    result = agent.analyze("AAPL", period_days=365)
